...
const fetchFollowers = async (): Promise<void> => {
  try {
    console.log("folower._id13442");
    if (!userRecipient.value) return;

    const response = await axios.get(`${API_BASE_URL}/followers/${userRecipient.value}`);
...
const vOnSubscribe = async (folower: any) => {
  try {
...
const vOnUnsubscribe = async (folower: any) => {
  try {
    
    console.log("folower._id1");
    const response = await axios.put(`${API_BASE_URL}/subscribe`, {
      userId: userId.value,
      pageId: folower._id,
    });
...
<template>
    <v-container class="followers-page">
      <v-card class="followers-card">
        <v-card-title class="text-h5 font-weight-bold text-center">Подписчики</v-card-title>
        <v-divider></v-divider>
        
        <v-card-text>
          <v-list v-if="followers.length > 0" class="pa-0">
            <v-list-item v-for="follower in followers" :key="follower._id" class="follower-item">
              ...
              <v-col cols="auto">
    <v-btn  color="blue" @click="() => follower.followers.includes(userId!) ? vOnUnsubscribe(follower) : vOnSubscribe(follower)">
      <v-icon v-if="follower.followers.includes(userId!)">mdi-account-check</v-icon>
      <v-icon v-else>mdi-account-plus</v-icon>
      {{ follower.followers.includes(userId!) ? "Отписаться" : "Подписаться" }}
    </v-btn>
  </v-col>
            </v-list-item>
        ...
        <v-card-actions class="justify-center">
          <v-btn color="primary" @click="navigateTo(Pages.UserPage, { params: { id: userRecipient } })">
