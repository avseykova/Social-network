...
const fetchSubscriptions = async (): Promise<void> => {
  try {
    ...
    const response = await axios.get(`${API_BASE_URL}/subscriptions/${userRecipient.value}`);
...
const vOnSubscribe = async (subscription: any) => {
  try {
...
const vOnUnsubscribe = async (subscription: any) => {
  try {
...
<template>
    <v-container class="followers-page">
      <v-card class="followers-card">
        <v-card-title class="text-h5 font-weight-bold text-center">Подписки</v-card-title>
        <v-divider></v-divider>
        
        <v-card-text>
          <v-list v-if="subscriptions.length > 0" class="pa-0">
            <v-list-item v-for="subscription in subscriptions" :key="subscription._id" class="follower-item">
              ...
              <v-col cols="auto">
    <v-btn  color="blue" @click="() => subscription.followers.includes(userId!) ? vOnUnsubscribe(subscription) : vOnSubscribe(subscription)">
      <v-icon v-if="subscription.followers.includes(userId!)">mdi-account-check</v-icon>
      <v-icon v-else>mdi-account-plus</v-icon>
      {{ subscription.followers.includes(userId!) ? "Отписаться" : "Подписаться" }}
    </v-btn>
  </v-col>
            </v-list-item>
